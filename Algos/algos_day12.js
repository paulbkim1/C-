...
class MinHeap {
    ...
    extract() {
        ...
        let idxOfNodeToShiftDown = this.heap;
        while (idxOfNodeToShiftDown < array.length-1) {
            const idxOfParent = this.idxOfParent(idxOfNodeToShiftDown);

            const isParentGreaterOrEqual =
            this.heap[idxOfParent] >= this.heap[idxOfNodeToShiftDown];

            if (isParentGreaterOrEqual) {
                break;
                }
        }
    }
...
test = new MinHeap()
test.insert(2).insert(4).insert(7).insert(10).insert(17).insert(20).insert(24)
